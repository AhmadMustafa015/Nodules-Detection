...
class MaskHead(nn.Module):
    ...
    def forward(self, detections, features):
        ...
        for detection in detections:
            ...
            print(b, z_start, y_start, x_start, z_end, y_end, x_end, cat)
            ...
            up3 = self.up3(up2)
            im = img[int(b), :, int(z_start):int(z_end), int(y_start):int(y_end), int(x_start):int(x_end)].unsqueeze(0)
            ...
            mask[int(z_start):int(z_end), int(y_start):int(y_end), int(x_start):int(x_end)] = logits
